...
module.exports = function(app, passport) {
        ...
        var huidigetappe = require('./girodata') //haal de huidige etappe op
        var etap=huidigetappe.etapstart;
        ...
        app.get('/giro/teamselectie', isLoggedIn, function(req, res) {
            if(new Date().getTime()<huidigetappe.girostart.getTime()){
        ...
        app.get('/giro/etappe*', isLoggedIn, function(req, res){
            ...
            etappe = parseInt(query,10); //String omzetten naar int (decimaal)
            if (isNaN(etappe)==true || etappe<1 || etappe>21){ //Kijken of het een nummer is en of het geen ongeldig nummer is
                res.redirect("/giro/etappe1")
            }else{ 
              if(etap>etappe){ //Etappe voorbij, naar resultatenpagina met alle info
